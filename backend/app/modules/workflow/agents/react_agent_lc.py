        ...
        try:
            ...
            async for chunk in self.agent_executor.astream(input_data, config):
                ...
                for node_name, node_data in chunk.items():
                    # create_agent names the reasoning node "model" (langchain 1.x);
                    # "agent" kept for older graph builds.
            ...
            # Build the final result from what streaming already produced
            if final_response:
                ...
                final_result = create_error_response(
                    "No response generated by LangGraph agent",
                    self._get_agent_name(),
                    thread_id=thread_id,
                )
            if llm_usage_entries:
                final_result["llm_usage"] = llm_usage_entries
